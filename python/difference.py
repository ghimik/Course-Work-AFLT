...
def process_state_pairs(dfa1, dfa2, state_map, queue, new_transitions):
    ...
    while queue:
        ...
        for symbol in dfa1.alphabet & dfa2.alphabet:
...
def mark_final_states(state_map):
    """Отмечает финальные состояния в автомате разности."""
    for (q1, q2), new_state in state_map.items():
        if (q1.is_final and not q2.is_final) or (not q1.is_final and q2.is_final):
...
def build_difference_automaton(dfa1: DFA, dfa2: DFA) -> DFA:
    ...
    mark_final_states(state_map)

    return DFA(new_states, dfa1.alphabet & dfa2.alphabet, new_transitions, new_start_state)
